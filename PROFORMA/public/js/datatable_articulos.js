$(document).ready(function() {
	...
	var table = $('#tb_articulos').DataTable( {
		"bDeferRender": true,
    ...
    //-----------------rellenar select marcas----------------
    $("#btn_nuevoArticulo").click(function(e){
    	e.preventDefault();
    	var options = $('#marcas option').size(); 

    	if(options <= 1){
    		var url = "/listarMarcas"
    		$.get(url,function(resul){
		 		$.each(resul, function(id,value){
 				$("#marcas").append('<option value="'+value["idMarca"]+'">'+value["nombreMarca"]+'</option>');
     			});
			})
    	}
    });
